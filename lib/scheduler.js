...
export function planRescue(todayEvents, now = new Date()) {
  ...
  return {
    ...
    message: heavy.length > 0
      ? `Hey, I got you. Shutting it down. I pushed all your heavy deep work (${heavy.length} block${heavy.length === 1 ? '' : 's'}) to ${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][nextWeekday(now).getDay()]} morning. Go grab some fresh air.`
      : "Hey, I got you. No heavy blocks left today — you're already in the clear. Go take a breather anyway. 🌿",
  };
}
